...
// Request interface
interface AnalyzeRecordingRequest {
  recordingUrl: string;
  userId: string;
  interviewId: string;
}
...
serve(async (req) => {
  ...
  try {
    ...
    const payload: AnalyzeRecordingRequest = await req.json();
    
    // Validate payload
    if (!payload.recordingUrl || !payload.userId || !payload.interviewId) {
      return new Response(
    ...
    // Get audio file
    console.log("Downloading audio file from URL:", payload.recordingUrl);
    const audioResponse = await fetch(payload.recordingUrl);
    if (!audioResponse.ok) {
    ...
    // Combine transcript and analysis results
    const result = {
      transcript,
      ...analysisResult
    };
    
    // Save the transcript and analysis results to the database
    ...
    console.error("Error in analyze-interview-recording:", error);
...
async function analyzeTranscript(
  ...
  try {
    ...
    try {
      ...
      return {
        feedback: "We encountered an issue processing your interview recording. The analysis may be incomplete.",
...
async function saveAnalysisResults(
  ...
  try {
    // Update the interview record with transcript and analysis results
